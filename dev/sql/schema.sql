drop database if exists cloud_oj;
...
create index idx_cid on contest_problem (contest_id);
create index idx_pid on contest_problem (problem_id);
create index idx_order on contest_problem (`order`);
...
select `c`.`contest_id`   AS `contest_id`,
       ...
       `cp`.`order`       AS `order`
from ((`contest_problem` `cp` join `problem` `p` on (`cp`.`problem_id` = `p`.`problem_id`))
    join `contest` `c` on (`c`.`deleted` = 0 and `cp`.`contest_id` = `c`.`contest_id`));
...
# 初始 ADMIN 用户
INSERT INTO user (uid, username, nickname, password, secret, role)
